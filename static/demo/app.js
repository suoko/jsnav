function init() {
  ...
  var map = new ffwdme.components.Leaflet({
    el: $("#map"),
    tileURL: tileURL,
    center: { lat: 49.90179, lng: 8.85723 }
  });
  ...
  window.widgets = {
    ...
    nextTurn: new ffwdme.components.NextStreet({
      parent: "#playground",
      grid: { x: 4, y: 11 }
    }),
    distance: new ffwdme.components.DistanceToNextTurn({
      parent: "#playground",
      grid: { x: 5, y: 1, w: 4 }
    }),

    // speed     : new ffwdme.components.Speed({ parent: '#playground', grid: { x: 1, y: 12 } }),
    destTime: new ffwdme.components.TimeToDestination({
      parent: "#playground",
      grid: { x: 4, y: 12 }
    }),
    destDist: new ffwdme.components.DistanceToDestination({
      parent: "#playground",
      grid: { x: 7, y: 12 }
    }),
    arrival: new ffwdme.components.ArrivalTime({
      parent: "#playground",
      grid: { x: 10, y: 12 },
      defaultUnit: ""
    }),
    arrow: new ffwdme.components.Arrow({
      parent: "#playground",
      grid: { x: 0, y: 11 }
    }),
    ...
    zoom: new ffwdme.components.Zoom({
      map: map,
      parent: "#playground",
      grid: { x: 0, y: 1, w: 4 }
    }),
    overview: new ffwdme.components.RouteOverview({
      map: map,
      parent: "#playground",
      grid: { x: 9, y: 0 }
    }),
  ...
  if (/debug=/.test(window.location.href)) {
    ...
    $("#views-toggle, #nav-info-trigger, #routing-trigger").removeClass(
      "hidden"
    );
  }
}
